...
const paymentLinks = {
  growth: {
    monthly: "https://link.omnidigitalsolutions.ai/payment-link/68a3480dcd9d3481f436217d",
    annual: "https://link.omnidigitalsolutions.ai/payment-link/68a3482bcd9d3458fc362181",
    monthlyPrice: 349,
    annualPrice: 3769,
    annualOriginal: 4188,
  },
  pro: {
    monthly: "https://link.omnidigitalsolutions.ai/payment-link/68a34c1867ee3b876b68530f",
    annual: "https://link.omnidigitalsolutions.ai/payment-link/68a34c5b613b1b5dbdcd34b1",
    monthlyPrice: 699,
    annualPrice: 7549,
    annualOriginal: 8388,
  },
...
export default function PlanCtaModal({ open, onClose, plan }: PlanCtaModalProps) {
  const config = paymentLinks[plan];

  const handleClick = (type: "monthly" | "annual") => {
    window.open(config[type], "_blank", "noopener,noreferrer");
    onClose();
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-xs sm:max-w-md">
        ...
        <div className="flex flex-col gap-6 mt-6">
          ...
          >
            <Calendar className="w-7 h-7 text-blue-400 mb-2" aria-hidden="true" />
            <span className="text-3xl font-extrabold text-blue-800 mb-1">
              £{config.monthlyPrice}/mo
            </span>
            <span className="text-xs font-medium text-gray-500 mb-1">Billed monthly</span>
            <span className="text-sm text-gray-500 mb-2">Flexible, cancel anytime</span>
            <span className="mt-2 inline-block px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-full text-base shadow transition">
              {plan === "growth" ? "Secure Growth" : "Secure Pro"}
            </span>
            <CheckCircle className="absolute right-5 top-5 w-6 h-6 text-green-500 opacity-0 group-hover:opacity-100 transition-opacity duration-150" aria-hidden="true" />
          </button>
          ...
          >
            ...
            <span className="text-xs font-medium text-gray-400 line-through mb-1">
              £{config.annualOriginal.toLocaleString()}
            </span>
            <span className="text-3xl font-extrabold text-indigo-800 mb-1">
              £{config.annualPrice.toLocaleString()}
            </span>
            <span className="text-xs font-medium text-gray-500 mb-1">Billed annually</span>
            <span className="text-sm text-gray-500 mb-2">Best for growing teams</span>
            <span className="mt-2 inline-block px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-full text-base shadow transition">
              {plan === "growth" ? "Secure Growth (Annual)" : "Secure Pro (Annual)"}
            </span>
            <CheckCircle className="absolute right-5 top-5 w-6 h-6 text-green-500 opacity-0 group-hover:opacity-100 transition-opacity duration-150" aria-hidden="true" />
          </button>
        </div>

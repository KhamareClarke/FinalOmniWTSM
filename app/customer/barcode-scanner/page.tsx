...
export default function CustomerBarcodeScannerPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("customerBarcodeSearchHistory");
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    if (typeof window !== "undefined") {
      localStorage.removeItem("customerBarcodeSearchHistory");
    }
  };
  ...
  const handleScan = async (code: string) => {
    ...
    try {
      ...
      if (data) {
        setProduct(data);
        showNotification(`Found product: ${data.name}`, "success");
      ...
      <main className="flex-1 w-full">
        <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight bg-gradient-to-r from-[#3456FF] to-[#8763FF] bg-clip-text text-transparent">
                Barcode Scanner
              </h1>
              <p className="text-gray-600 mt-1">
                Scan or enter barcodes to view product details
              </p>
            </div>
            {isMounted && (
